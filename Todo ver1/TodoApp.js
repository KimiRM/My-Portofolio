...
const AddSectionTask_Title = document.getElementById(
  "AddSectionTask-TitleInput",
);
const AddSectionTask_Desc = document.getElementById("AddSectionTask-DescInput");
const AddSectionTask_StartTime = document.getElementById(
  "AddSectionTask-StartTime-input",
);
const AddSectionTask_EndTime = document.getElementById(
  "AddSectionTask-EndTime-input",
);

const AddSectionEvent_Title = document.getElementById(
  "AddSectionEvent-TitleInput",
);
const AddSectionEvent_Desc = document.getElementById(
  "AddSectionEvent-DescInput",
);
const AddSectionEvent_Date = document.getElementById(
  "AddSectionEvent-Date-input",
);
const AddSectionEvent_DurDiv = document.getElementById(
  "AddSectionEvent-Duration-options",
);
...
const FilterDateFrom = document.getElementById("filter-DateFrom-input");
const FilterDateTo = document.getElementById("filter-DateTo-input");
...
class TodoAPP {
  constructor() {
    ...
    FilterBtn.addEventListener("click",this.FilterSectionShow);

    Overlay.addEventListener("click",(e) => {
      if (e.target === Overlay) {
        this._CloseAddSection();
        this._CloseSearchSection();
  ...
  _AddTask = () => {
    try {
      ...
      const prog = this._GetProgressTask();

      this._CreatTask(t, d, st, et, p, c,prog);
  ...
  _AddEvent = () => {
    try {
      ...
      const prog = this._GetProgressEvent();

      this._CreatEvent(t, d, date, dur, p, c,prog);
      this._CloseAddSection();
    } catch (err) {
      console.log(err.message);
    }
  ...
  _ShowActivities = () => {
    ...
    if (this.$searchedItem) {
      let activities = this.$itemsOnBoard.filter((a) => {
        return (
          a.title.includes(this.$searchedItem) ||
          a.descript.includes(this.$searchedItem)
        );
      });
  ...
  _CreateTableBodyRows(obj, colValues, id) {
    ...
    this._showProgress(progressCol, id);
    tr.appendChild(progressCol);
  ...
  _showPriority(td, id) {
    ...
    td.appendChild(priorityWrapper);
  }
...
_showProgress(td, id) {
    ...
    const existingWrapper = td.querySelector('.progress-wrapper');
    if (existingWrapper) {
    ...
    this.styler._styleProgressDiv(wrapper);

    const canvas = document.createElement('canvas');
    canvas.id = 'progCanvas';
    canvas.width = 60;
    canvas.height = 60;
    ...
    ctx.strokeStyle = "#F5cac3";
    ctx.lineWidth = 12;
    ctx.lineCap = 'round';
    ctx.stroke();

    ctx.beginPath();
    let endAngle = (item.progress / 100) * 2 * Math.PI;
    ctx.arc(centerX, centerY, radius, -Math.PI/2, endAngle - Math.PI/2);
    ctx.strokeStyle = "#f28482";
    ctx.lineWidth = 12;
    ctx.lineCap = 'round';
    ctx.stroke();
  ...
  _EditActivity = (row, activity) => {
    ...
    if (activity.type === "Task") {
      ...
      AddSectionTask_StartTime.value = moment(activity.Start).format(
        "YYYY-MM-DDTHH:mm",
      );
      AddSectionTask_EndTime.value = moment(activity.End).format(
        "YYYY-MM-DDTHH:mm",
      );
      ...
      AddSectionAddBtn.addEventListener("click", () =>
        this._UpdateTask(activity.id),
      );
    } else {
      AddSectionEventDiv.style.display = "block";
      AddSectionTaskDiv.style.display = "none";
      ...
      AddSectionEvent_Date.value = moment(activity.date).format(
        "YYYY-MM-DDTHH:mm",
      );
  ...
  _UpdateTask = (id) => {
    try {
      ...
      if (index !== -1) {
        ...
        this.$Activities[index].priority = p;
        this.$Activities[index].category = c;
        this.$Activities[index].progress = prog;
      }
